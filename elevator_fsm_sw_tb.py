...
def check_memory(variable):
    global SRAM_LIMIT
    estimated_bytes = len(variable)

    if estimated_bytes > SRAM_LIMIT:
        print("Exceeding 2KB")
        return False
    else:
...
def test(test_name, initial_requests, start_floor = 0):
    ...
    dev.current_state = dev.state_IDLE
    dev.direction = dev.direction_IDLE
    ...
    for tick in range(100):
        ...
        if dev.current_state == dev.state_IDLE and dev.requests == 0 and dev.current_floor_idx == 0:
